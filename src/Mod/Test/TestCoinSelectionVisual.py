...
class TestCoinSelectionVisual(unittest.TestCase):
    ...
    def _center_pixel_color(self):
        ...
        try:
            self.view.saveImage(str(path), 256, 256, "White")
            image = QtGui.QImage(str(path))
            self.assertFalse(image.isNull(), f"Could not read rendered view image: {path}")
            ...
            self.assertGreater(image.width(), 0, "Rendered view image has zero width")
            self.assertGreater(image.height(), 0, "Rendered view image has zero height")
