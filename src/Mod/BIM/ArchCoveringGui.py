        ...
        *Edit path* (double-click in Tree View): Tree.cpp opens an "Edit Covering" transaction
        ...
        def _setup_bound_spinbox(self, prop_name, tooltip=None, initial_value=None):
            ...
            if initial_value is not None:
                self._set_spinbox_quantity(sb, initial_value)
            elif "." not in prop_name:
                self._set_spinbox_quantity(sb, getattr(self.template.buffer, prop_name))
            FreeCADGui.ExpressionBinding(sb).bind(self.template.buffer, prop_name)
            self._bound_spinboxes.append((sb, prop_name))
            return sb

        def _set_spinbox_quantity(self, spinbox, quantity):
            spinbox.setProperty("unit", quantity.getUserPreferred()[2])
            spinbox.setProperty("rawValue", quantity.Value)

        def _spinbox_quantity(self, spinbox, unit):
            return FreeCAD.Units.Quantity(spinbox.property("rawValue"), unit)
        ...
        def _on_alignment_mode_changed(self, is_preset):
            ...
            if is_preset:
                self._set_spinbox_quantity(
                    self.sb_u_off, FreeCAD.Units.Quantity(0, FreeCAD.Units.Length)
                )
                self._set_spinbox_quantity(
                    self.sb_v_off, FreeCAD.Units.Quantity(0, FreeCAD.Units.Length)
                )
        ...
        def _set_rotation(self, angle_deg):
            ...
            self._set_spinbox_quantity(
                self.sb_rot, FreeCAD.Units.Quantity(angle_deg, FreeCAD.Units.Angle)
            )
        ...
        def _set_custom_offset(self, u_off, v_off):
            ...
            self._set_spinbox_quantity(
                self.sb_u_off, FreeCAD.Units.Quantity(u_off, FreeCAD.Units.Length)
            )
            self._set_spinbox_quantity(
                self.sb_v_off, FreeCAD.Units.Quantity(v_off, FreeCAD.Units.Length)
            )
        ...
        def _init_pick_rotation(self):
            ...
            """
            try:
                self._pick_rotation_deg = self.sb_rot.property("rawValue")
            except Exception:
        ...
        def _sync_ui_to_target(self):
            ...
            # Sync numeric properties directly from rawValue to avoid PySide Base::Quantity
            # conversion failures when reading the QuantitySpinBox "value" property.
            obj.TileLength = self._spinbox_quantity(self.sb_length, FreeCAD.Units.Length)
            obj.TileWidth = self._spinbox_quantity(self.sb_width, FreeCAD.Units.Length)
            obj.TileThickness = self._spinbox_quantity(self.sb_thick, FreeCAD.Units.Length)
            obj.JointWidth = self._spinbox_quantity(self.sb_joint, FreeCAD.Units.Length)
            obj.Rotation = self._spinbox_quantity(self.sb_rot, FreeCAD.Units.Angle)
            ...
            obj.AlignmentOffset = FreeCAD.Vector(
                self.sb_u_off.property("rawValue"),
                self.sb_v_off.property("rawValue"),
                0.0,
            )
            ...
            obj.StaggerCustom = self._spinbox_quantity(
                self.sb_stagger_custom, FreeCAD.Units.Length
            )

            obj.BorderSetback = self._spinbox_quantity(self.sb_setback, FreeCAD.Units.Length)
